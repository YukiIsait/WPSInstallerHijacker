#include "Win32Common/Exception.h"
...
FileSystemWatcher::FileSystemWatcher(std::wstring_view fileName, size_t bufferSize): buffer(bufferSize),
                                                                                     handle(::CreateFileW(fileName.data(), GENERIC_READ | GENERIC_WRITE | FILE_LIST_DIRECTORY,
                                                                                                        ...
                                                                                                        nullptr)) {
    Win32::Exception::ThrowLastErrorIf(handle == INVALID_HANDLE_VALUE);
}
...
void FileSystemWatcher::Watch(bool watchSubtree, Change filter, std::function<bool(Action action, const std::wstring& fileName)> handler) {
    DWORD bytesReturned;
    while (true) {
        Win32::Exception::ThrowLastErrorIf(!::ReadDirectoryChangesW(handle, &buffer[0], (DWORD) buffer.size(), watchSubtree, (DWORD) filter, &bytesReturned, nullptr, nullptr));
        Win32::Exception::ThrowLastErrorIf(bytesReturned == 0, ERROR_INSUFFICIENT_BUFFER);
